    ...
    */
    ...
    public static long makeMove(long board,String move,char type){
        if(Character.isDigit(move.charAt(3))){
            ...
            int start = (Character.getNumericValue(move.charAt(0))*8)+(Character.getNumericValue(move.charAt(1)));
            int end = (Character.getNumericValue(move.charAt(2))*8)+(Character.getNumericValue(move.charAt(3)));
            if(((board>>start)&1)==1){//is the piece mentioned is on this bit board or not
                board&=~(1L<<start);    //remove piece
                board|=(1L<<end);       //put piece
            }else{
                board&=~(1L<<end);  //removes all the pieces at the end location (eg for capturing)
            }
    ...
    public static String possibleWP(long WP,long BP,long EP){
        ...
        //y1,y2.Promotion Type,"P"
        PAWN_MOVES=(WP>>7)&NOT_MY_PIECES&OCCUPIED&RANK_8&~FILE_A;//Pawn promotion by right capture
        possibility=PAWN_MOVES&~(PAWN_MOVES-1);
        while (possibility != 0)
        {
            int index=Long.numberOfTrailingZeros(possibility);
